...
export const JSONEditorModal: React.FC<JSONEditorModalProps> = ({ 
  ...
  return (
    ...
    >
      <div 
        className="bg-white rounded-lg shadow-xl w-full max-w-4xl flex flex-col overflow-hidden max-h-[90vh]" 
        onClick={e => e.stopPropagation()}
      >
        <header className="flex items-center justify-between p-4 border-b border-gray-200">
          <div>
            <h2 id="json-editor-title" className="text-lg font-semibold text-gray-800">
              Edit JSON: {variableName}
            </h2>
            <p className="text-xs text-gray-500 mt-1">Type: {variableType}</p>
          </div>
          <button 
            onClick={onClose} 
        ...
        <main className="p-4 flex-grow bg-gray-50 flex flex-col min-h-0">
          <div className="mb-3 flex items-center justify-between">
            <div className="text-xs text-gray-600">
              <p>Edit JSON value. Press <kbd className="bg-gray-200 px-1.5 py-0.5 rounded text-xs">Ctrl/Cmd + S</kbd> to save, <kbd className="bg-gray-200 px-1.5 py-0.5 rounded text-xs">Esc</kbd> to cancel.</p>
            </div>
            <button
