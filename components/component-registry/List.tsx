...
export const ListComponentRenderer: React.FC<ListComponentRendererProps> = ({
  ...
  const data = useMemo(() => {
    ...
    if (typeof dataExpression === 'string') {
      try {
        ...
        console.error('Error evaluating list data:', error);
        return [];
      }
    }
  ...
  if (mode === 'preview') {
    console.log('List Preview Debug:', {
      dataLength: data.length,
      data: data,
      templateChildrenCount: templateChildren.length,
      templateChildren: templateChildren.map(c => ({ id: c.id, type: c.type })),
