...
const PanelRenderer: React.FC<{
  ...
  const align = useJavaScriptRenderer(p.alignItems, evaluationScope, 'start');
...
const PanelProperties: React.FC<{
  ...
  const align = p.alignItems || 'start';
    ...
    <>
      <div className="mb-3">
        ...
        <div className="flex gap-2">
          <button onClick={() => updateProp('direction', 'horizontal')} className={`p-2 rounded-md border ${dir === 'horizontal' ? 'bg-blue-50 border-blue-400' : 'bg-white border-gray-200'}`} title="Horizontal" aria-pressed={dir === 'horizontal'}>
          ...
          <button onClick={() => updateProp('direction', 'vertical')} className={`p-2 rounded-md border ${dir === 'vertical' ? 'bg-blue-50 border-blue-400' : 'bg-white border-gray-200'}`} title="Vertical" aria-pressed={dir === 'vertical'}>
      ...
      <div className="mb-3">
        ...
        <div className="flex gap-2">
          {JustifyOptions.map(opt => (
            <button key={opt.value} onClick={() => updateProp('justifyContent', opt.value as any)} className={`p-2 rounded-md border ${justify === opt.value ? 'bg-blue-50 border-blue-400' : 'bg-white border-gray-200'}`} title={opt.title} aria-pressed={justify === opt.value}>
              {opt.icon}
            </button>
          ))}
        </div>
      ...
      <div className="mb-3">
        ...
        <div className="flex gap-2">
          {AlignOptions.map(opt => (
            <button key={opt.value} onClick={() => updateProp('alignItems', opt.value as any)} className={`p-2 rounded-md border ${align === opt.value ? 'bg-blue-50 border-blue-400' : 'bg-white border-gray-200'}`} title={opt.title} aria-pressed={align === opt.value}>
              {opt.icon}
            </button>
          ))}
        </div>
