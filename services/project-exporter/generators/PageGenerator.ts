...
type ${pageName}Props = {
    theme: any;
    dataStore: any;
    updateDataStore: (key: string, value: any) => void;${appDef.variables.length > 0 ? '\n    ' + appDef.variables.map(v => `${v.name}: any;`).join('\n    ') + '\n    ' + appDef.variables.map(v => `set${toPascalCase(v.name)}: React.Dispatch<React.SetStateAction<any>>;`).join('\n    ') : ''}
};

export const ${pageName}: React.FC<${pageName}Props> = ({ theme, dataStore, updateDataStore${appDef.variables.length > 0 ? ', ' + appDef.variables.map(v => v.name).join(', ') + ', ' + appDef.variables.map(v => `set${toPascalCase(v.name)}`).join(', ') : ''} }) => {
