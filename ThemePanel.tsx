...
const ThemePropInput: React.FC<{ 
  ...
  <div className="mb-3">
    ...
    <div className="flex items-center">
      {type === 'color' && <input type="color" value={value} onChange={e => onUpdate(category, prop, e.target.value)} className="p-1 h-8 w-8 block bg-white border border-r-0 border-gray-300 cursor-pointer rounded-l-md" title={`Select ${label} color`} />}
      <input
        type="text"
        value={value}
        onChange={e => onUpdate(category, prop, e.target.value)}
        className={`w-full bg-gray-50 border border-gray-300 p-2 text-sm text-gray-800 focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 ${type === 'color' ? 'rounded-r-md' : 'rounded-md'}`}
      />
    </div>
  </div>
);
  ...
  return (
    <div className="flex flex-col h-full">
        <div className="p-3 overflow-y-auto">
            <h3 className="text-sm font-bold text-gray-500 uppercase tracking-wider mb-4">App Theme</h3>
            
            <div className="p-3 mb-4 bg-gray-50 border border-gray-200 rounded-md">
                <h4 className="text-xs font-semibold text-gray-600 mb-2">Apply Global Theme</h4>
                <select 
                ...
                <button 
                    ...
                    className="w-full bg-blue-600 text-white text-sm font-semibold py-2 rounded-md hover:bg-blue-700 disabled:bg-gray-400"
                >
                    Apply Theme
                </button>
            </div>
            ...
            <div className="space-y-4">
                <div>
                    <h4 className="text-xs font-semibold text-gray-600 mb-2">Colors (App Specific)</h4>
                 ...
                 <div>
                    <h4 className="text-xs font-semibold text-gray-600 mb-2">Fonts</h4>
                    <ThemePropInput label="Family" value={theme.font.family} category="font" prop="family" onUpdate={onUpdateTheme} />
                </div>
                <div>
                    <h4 className="text-xs font-semibold text-gray-600 mb-2">Borders</h4>
                    ...
                    <div className="mb-3">
                        ...
                        <select value={theme.border.style} onChange={e => onUpdateTheme('border', 'style', e.target.value)} className="w-full p-2 border border-gray-300 rounded-md text-sm">
                 ...
                 <div>
                    <h4 className="text-xs font-semibold text-gray-600 mb-2">Radius</h4>
