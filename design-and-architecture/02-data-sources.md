...
When a user performs an action (e.g., clicking a button with a "Create Record" action), the `useAppData` hook uses the `dataSourceName` from the button's properties to find the `DataSourceInstance`. It then uses the instance's `providerId` to look up the correct provider in the `dataSourceRegistry` and call its `.createRecord()` method.
