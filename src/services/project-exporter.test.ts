...
describe('Project Exporter', () => {
  ...
  describe('Page Generator', () => {
    ...
    it('generates code for updateData action', () => {
        const appWithUpdateBtn = {
            ...mockApp,
            components: [{
                id: 'btn2', type: ComponentType.BUTTON, pageId: 'page1',
                props: { actionType: 'updateData', actionUpdateKey: 'foo', actionUpdateValue: '{{ 123 }}' } as any
            }]
        } as AppDefinition;
        const code = generatePageTsx(appWithUpdateBtn.pages[0], appWithUpdateBtn.components, appWithUpdateBtn);
        expect(code).toContain("updateDataStore('foo', 123)");
    });
    ...
    it('generates containers recursively', () => {
        const appWithContainer = {
            ...mockApp,
            components: [
                { id: 'panel1', type: ComponentType.PANEL, pageId: 'page1', props: { x:0, y:0, width:200, height:200 } as any },
                { id: 'child1', type: ComponentType.LABEL, pageId: 'page1', parentId: 'panel1', props: { text: 'Child' } as any }
            ]
        } as AppDefinition;
